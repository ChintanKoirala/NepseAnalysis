...
packages = ["nepse-scraper", "xlsxwriter", "gitpython", "pandas", "matplotlib", "joblib", "beautifulsoup4"]
...
first['Pct_from_52W_High_Sign'] = first['Pct_from_52W_High'].apply(
    lambda x: f"+{x-100:.2f}%" if x >= 100 else f"-{100-x:.2f}%"
)
first['Pct_from_52W_Low_Sign'] = first['Pct_from_52W_Low'].apply(
    lambda x: f"+{x-100:.2f}%" if x >= 100 else f"-{100-x:.2f}%"
)
...
try:
    ...
    secondss.columns = [
        ...
        'Percent Change', 'Volume', '52W_High', '52W_Low'
    ]
...
all_valid_crossovers['Pct_from_52W_High_Sign'] = all_valid_crossovers['Pct_from_52W_High'].apply(
    lambda x: f"+{x-100:.2f}%" if x>=100 else f"-{100-x:.2f}%"
)
all_valid_crossovers['Pct_from_52W_Low_Sign'] = all_valid_crossovers['Pct_from_52W_Low'].apply(
    lambda x: f"+{x-100:.2f}%" if x>=100 else f"-{100-x:.2f}%"
)
